...
class Communication():
	...
	def _neighbor_liveness_callback(self, msg: Header):
		...
		if not msg.frame_id==self.parent_node.frame_id: self.parent_node.last_seen_neighbors[int(msg.frame_id)] = self.parent_node.get_clock().now()
		if not count==len(list(self.parent_node.last_seen_neighbors.keys())): self.referendum_conducting()

	def _check_neighbor_timeouts(self):
		now = self.parent_node.get_clock().now()
		timed_out_neighbors: Set[int] = set()
	...
	def referendum_conducting(self):
		...
		if leader_id != self.parent_node.leader_id:
			...
			self.parent_node.get_logger().info(f"This is drone {self.parent_node.frame_id} and leader is {leader_id}")
			...
			VehicleLocalPosition, f"/px4_{leader_id}/fmu/out/vehicle_local_position", self.vehicle_local_position_callback, self.qos_profile)
	...
	def become_leader(self, ):
		self.parent_node.get_logger().info('this is the leader')
