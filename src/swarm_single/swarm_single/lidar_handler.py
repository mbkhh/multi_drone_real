...
class lidarHandler():
	def __init__(self, parent_node: Node):
		...
		self.center_ranges = []
		self.center_angles = []
		self.tf_broadcaster = TransformBroadcaster(parent_node)

	def scan_callback(self, msg):

		euler = tf_transformations.euler_from_quaternion([self.parent_node.navigation.current_pos[3], self.parent_node.navigation.current_pos[4], self.parent_node.navigation.current_pos[5], self.parent_node.navigation.current_pos[6]])
		...
		if cluster_centers_cartesian:
			#self.parent_node.get_logger().info(f"Publishing {len(self.cluster_centers_cartesian)} obstacle TF frames.")
			
			transforms_to_publish = []
			for i, center_point in enumerate(self.cluster_centers_cartesian):
				t = TransformStamped()

				t.header.stamp = self.parent_node.get_clock().now().to_msg()
				
				t.header.frame_id = f"{self.parent_node.px4_model}_{self.parent_node.frame_id}/odom"
				
				t.child_frame_id = f'obstacle_{self.parent_node.frame_id}_{i}'

				t.transform.translation.x = center_point[0]
				t.transform.translation.y = center_point[1]
