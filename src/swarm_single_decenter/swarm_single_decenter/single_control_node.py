...
from swarm_single_decenter.navigation import navigation
# from swarm_single_decenter.lidar_handler import lidarHandler
...
class SingleControlNode(Node):
    ...
    def __init__(self):
        ...
        # These legacy fields remain only because copied helper code and the
        # Status schema contain them. No leader is elected in this package.
        self.leader_id = None
        ...
        self.declare_parameter('max_mission_leg_distance', 50.0)
        self.max_mission_leg_distance = float(
            self.get_parameter('max_mission_leg_distance')
            .get_parameter_value().double_value
        )
        if (
            not math.isfinite(self.max_mission_leg_distance)
            or self.max_mission_leg_distance <= 0.0
        ):
            raise ValueError(
                'max_mission_leg_distance must be finite and positive.'
            )
        ...
        self.get_logger().info(
            f'Decentralized control node initialized for drone {self.drone_id}.'
        )
        self.get_logger().info(
            'Decentralized collision avoidance: '
            f'neighbor_distance={self.navigation.neighbor_dist:.2f} m, '
            f'radius={self.navigation.radius:.2f} m per drone, '
            f'time_horizon={self.navigation.time_horizon:.2f} s.'
        )
    ...
    def start_mission(
        ...
        """Validate and start this drone's independent waypoint mission."""
        ...
        for index, point in enumerate(points):
            ...
            if leg_distance > self.max_mission_leg_distance:
                self.get_logger().error(
                    f'Mission rejected: waypoint {index + 1} has a '
                    f'{leg_distance:.2f} m horizontal leg, exceeding the '
                    f'{self.max_mission_leg_distance:.2f} m mission-leg limit.'
                )
                return False
    ...
    def activate_current_mission_waypoint(self):
        """Activate one waypoint from the already validated mission."""
        ...
        # The complete mission was validated up front, including its separate
        # mission-leg bound. Keep the smaller interactive set_goal bound for
        # station commands without rejecting this file's legitimate long legs.
        self.set_local_goal(target)

        self.mission_target = list(target)
        self.mission_target_yaw = target_yaw
        if target_yaw is not None:
            # self.yaw is this drone's independent local PX4/NED setpoint.
    ...
    def swarm_state_callback(self, msg):
        """Cache every peer's directly shared world-frame state."""
        try:
        ...
        self.last_seen_neighbors[peer_id] = received_at
    ...
    def request_relative_yaw(self, delta_degrees):
        ...
        if self.state != DroneState.TAKEOFF:
            self.get_logger().warning(
                'YAW rejected: this drone must be armed in Offboard TAKEOFF '
