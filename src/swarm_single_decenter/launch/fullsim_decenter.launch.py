"""Launch three independent SITL controllers and their ground station."""
...
def generate_launch_description():
    ...
    # Decentralized station: every controller connects to it directly.
    ...
    station_node = Node(
        package='swarm_station_decenter',
        executable='station',
        name='decentralized_station',
        output='screen',
        emulate_tty=True,
    ...
    for i in range(drone_count):
        ...
        single_control_node = Node(
            package='swarm_single_decenter',
            executable='control_node',
            name=f'control_node_d{str(instance_id)}',
            parameters=[{
                'frame_id': instance_id,
                # Each SITL estimator has its own local origin. Anchor it at
                # the matching Gazebo spawn point to create one shared ENU
                # world for the directly exchanged local-state messages.
        ...
        # Controllers exchange Odometry directly; no TF publisher is needed.
